...
from app.schemas.ai import (
    ...
    ProfanityRegionPayload,
)
from app.services import analyses_service, censorship_service
...
# --- Profanity detection (Phase 1: exact matcher, BUILT_IN only) -------------
#
# Mirrors the filler-detection orchestration above. The pieces that differ:
# - calls `censorship_service.detect_profanity` instead of `detect_fillers`
# - produces `ProfanityDetectionResult` instead of `FillerDetectionResult`
# - persisted op type is `ai_detect_profanity`
# - emits its own structured audit line so cost/regions are queryable per feature
#
# Transcript cache is shared: both features call `analyses_service` with the
# same `kind="transcript"`, so the second feature to run on a given asset is
# effectively free (cached row reused).


async def enqueue_detect_profanity(
    db: AsyncSession,
    user: User,
    asset_id: str,
) -> Operation:
    ...
    from app.workers.ai_worker import run_ai_detect_profanity_actor
    run_ai_detect_profanity_actor.send(operation.id)

    return operation


def run_ai_detect_profanity_job(db: Session, operation_id: str) -> None:
    asyncio.run(_run_ai_detect_profanity_job_async(db, operation_id))


async def _run_ai_detect_profanity_job_async(db: Session, operation_id: str) -> None:
    now = datetime.utcnow()
    wid = worker_id()
    res = db.execute(
        update(Operation)
    ...
    try:
        ...
        result = ProfanityDetectionResult(
            ...
            regions=[
                ProfanityRegionPayload(
                    start=r.start, end=r.end, text=r.text,
                    wordIndex=r.word_index, confidence=r.confidence,
                    category=r.category, matchedBy=r.matched_by,
                ) for r in regions
            ],
            costUsd=float(transcript.cost_usd) if transcript.cost_usd is not None else None,
            modelVersion=transcript.model_version,
        ...
        logger.info(
            "ai_detect_profanity done op=%s user=%s duration=%.1fs regions=%d cost=%s model=%s",
            operation_id, op.user_id, transcript.duration_sec,
            len(regions), transcript.cost_usd, transcript.model_version,
        )
    except TranscriptionError as e:
